import type { InstructionInfo } from '@rawrs/architecture/Target';

import { Opcodes } from '../opcodes';

export const sar: InstructionInfo = {
  identifier: 'sar',
  name: 'Shift Arithmetic Right',
  description:
    '`SAR` and `SHR` shift the bits of the operand downward. The low-order bit is shifted into the carry flag. The effect is to divide the operand by 2. `SAR` performs a signed divide: the high-order bit remains the same. `SHR` performs an unsigned divide: the high-order bit is set to 0.\n\nThe shift is repeated the number of times indicated by the second operand, which is either an immediate number or the contents of the `CL` register. To reduce the maximum execution time, the 286 does not allow shift counts greater than 31. If a shift count greater than 31 is attempted, only the bottom five bits of the shift count are used. The 8086 uses all 8 bits of the shift count.\n\nThe overflow flag is set only if the single-shift forms of the instructions are used. For left shifts, it is set to 0 if the high bit of the answer is the same as the result carry flag (i.e., the top two bits of the original operand were the same); it is set to 1 if they are different. For SAR it is set to 0 for all single shifts. For SHR, it is set to the high-order bit of the original operand. Neither flag bit is modified when the count value is zero.',
  modifies: ['CF', 'ZF', 'PF', 'SF'],
  undefined: ['AF'],
  macros: {
    ALU8_OP: [
      'alu_result = a:i8 >> b',
      'flag_op = ${FLAG_OP_ALU} | ${FLAG_OP_8BIT} | ${FLAG_OP_SHIFT_RIGHT} | ${FLAG_OP_SIGNED}',
    ],
    ALU16_OP: [
      'alu_result = a:i16 >> b',
      'flag_op = ${FLAG_OP_ALU} | ${FLAG_OP_16BIT} | ${FLAG_OP_SHIFT_RIGHT} | ${FLAG_OP_SIGNED}',
    ],
  },
  locals: [
    {
      identifier: 'effective_address',
      name: 'Effective Address',
      size: 32,
    },
  ],
  forms: [
    // 0xD0 /7 - SAR eb, 1
    {
      operation: [
        'effective_address = ${MOD_RM_SEGMENT} + %{DISP}',
        'a = RAM:u8[effective_address]',
        'b = 1',
        '${ALU8_OP}',
        'RAM:u8[effective_address] = alu_result',
      ],
      modifies: ['OF', 'CF', 'ZF', 'PF', 'SF'],
      opcode: [
        Opcodes.ROL_ROR_RCL_RCR_SAL_SAR_SHR_EB,
        'ModRM_110_111_00',
        'DISP_i16',
      ],
      operandSize: 8,
      cycles: 7,
    },
    {
      operation: [
        'effective_address = ${MOD_RM_SEGMENT} + ${MOD_RM_OFFSET}',
        'a = RAM:u8[effective_address]',
        'b = 1',
        '${ALU8_OP}',
        'RAM:u8[effective_address] = alu_result',
      ],
      modifies: ['OF', 'CF', 'ZF', 'PF', 'SF'],
      opcode: [Opcodes.ROL_ROR_RCL_RCR_SAL_SAR_SHR_EB, 'ModRM_rm_111_00'],
      operandSize: 8,
      cycles: 7,
    },
    {
      operation: [
        'effective_address = ${MOD_RM_SEGMENT} + ${MOD_RM_OFFSET} + %{DISP}',
        'a = RAM:u8[effective_address]',
        'b = 1',
        '${ALU8_OP}',
        'RAM:u8[effective_address] = alu_result',
      ],
      modifies: ['OF', 'CF', 'ZF', 'PF', 'SF'],
      opcode: [
        Opcodes.ROL_ROR_RCL_RCR_SAL_SAR_SHR_EB,
        'ModRM_rm_111_01',
        'DISP_i8',
      ],
      operandSize: 8,
      cycles: 7,
    },
    {
      operation: [
        'effective_address = ${MOD_RM_SEGMENT} + ${MOD_RM_OFFSET} + %{DISP}',
        'a = RAM:u8[effective_address]',
        'b = 1',
        '${ALU8_OP}',
        'RAM:u8[effective_address] = alu_result',
      ],
      modifies: ['OF', 'CF', 'ZF', 'PF', 'SF'],
      opcode: [
        Opcodes.ROL_ROR_RCL_RCR_SAL_SAR_SHR_EB,
        'ModRM_rm_111_10',
        'DISP_i16',
      ],
      operandSize: 8,
      cycles: 7,
    },
    {
      operation: [
        'a = ${MOD_RM_RM8}',
        'b = 1',
        '${ALU8_OP}',
        '${MOD_RM_RM8} = alu_result',
      ],
      modifies: ['OF', 'CF', 'ZF', 'PF', 'SF'],
      opcode: [Opcodes.ROL_ROR_RCL_RCR_SAL_SAR_SHR_EB, 'ModRM_rm_111_11'],
      operandSize: 8,
      cycles: 2,
    },
    // 0xD2 /7 - SAR eb, CL
    {
      operation: [
        'next if (CL & 0x1f) == 0',
        'effective_address = ${MOD_RM_SEGMENT} + %{DISP}',
        'a = RAM:u8[effective_address]',
        'b = CL & 0x1f',
        '${ALU8_OP}',
        'RAM:u8[effective_address] = alu_result',
      ],
      undefined: ['AF', 'OF'],
      opcode: [
        Opcodes.ROL_ROR_RCL_RCR_SAL_SAR_SHR_EB_CL,
        'ModRM_110_111_00',
        'DISP_i16',
      ],
      operandSize: 8,
      cycles: 8,
    },
    {
      operation: [
        'next if (CL & 0x1f) == 0',
        'effective_address = ${MOD_RM_SEGMENT} + ${MOD_RM_OFFSET}',
        'a = RAM:u8[effective_address]',
        'b = CL & 0x1f',
        '${ALU8_OP}',
        'RAM:u8[effective_address] = alu_result',
      ],
      undefined: ['AF', 'OF'],
      opcode: [Opcodes.ROL_ROR_RCL_RCR_SAL_SAR_SHR_EB_CL, 'ModRM_rm_111_00'],
      operandSize: 8,
      cycles: 8,
    },
    {
      operation: [
        'next if (CL & 0x1f) == 0',
        'effective_address = ${MOD_RM_SEGMENT} + ${MOD_RM_OFFSET} + %{DISP}',
        'a = RAM:u8[effective_address]',
        'b = CL & 0x1f',
        '${ALU8_OP}',
        'RAM:u8[effective_address] = alu_result',
      ],
      undefined: ['AF', 'OF'],
      opcode: [
        Opcodes.ROL_ROR_RCL_RCR_SAL_SAR_SHR_EB_CL,
        'ModRM_rm_111_01',
        'DISP_i8',
      ],
      operandSize: 8,
      cycles: 8,
    },
    {
      operation: [
        'next if (CL & 0x1f) == 0',
        'effective_address = ${MOD_RM_SEGMENT} + ${MOD_RM_OFFSET} + %{DISP}',
        'a = RAM:u8[effective_address]',
        'b = CL & 0x1f',
        '${ALU8_OP}',
        'RAM:u8[effective_address] = alu_result',
      ],
      undefined: ['AF', 'OF'],
      opcode: [
        Opcodes.ROL_ROR_RCL_RCR_SAL_SAR_SHR_EB_CL,
        'ModRM_rm_111_10',
        'DISP_i16',
      ],
      operandSize: 8,
      cycles: 8,
    },
    {
      operation: [
        'next if (CL & 0x1f) == 0',
        'a = ${MOD_RM_RM8}',
        'b = CL & 0x1f',
        '${ALU8_OP}',
        '${MOD_RM_RM8} = alu_result',
      ],
      undefined: ['AF', 'OF'],
      opcode: [Opcodes.ROL_ROR_RCL_RCR_SAL_SAR_SHR_EB_CL, 'ModRM_rm_111_11'],
      operandSize: 8,
      cycles: 5,
    },
    // 0xC0 /7 - SAR eb, db
    {
      operation: [
        'next if (%{IMM} & 0x1f) == 0',
        'effective_address = ${MOD_RM_SEGMENT} + %{DISP}',
        'a = RAM:u8[effective_address]',
        'b = %{IMM} & 0x1f',
        '${ALU8_OP}',
        'RAM:u8[effective_address] = alu_result',
      ],
      undefined: ['AF', 'OF'],
      opcode: [
        Opcodes.ROL_ROR_RCL_RCR_SAL_SAR_SHR_EB_DB,
        'ModRM_110_111_00',
        'DISP_i16',
        'IMM_u8',
      ],
      operandSize: 8,
      cycles: 8,
    },
    {
      operation: [
        'next if (%{IMM} & 0x1f) == 0',
        'effective_address = ${MOD_RM_SEGMENT} + ${MOD_RM_OFFSET}',
        'a = RAM:u8[effective_address]',
        'b = %{IMM} & 0x1f',
        '${ALU8_OP}',
        'RAM:u8[effective_address] = alu_result',
      ],
      undefined: ['AF', 'OF'],
      opcode: [
        Opcodes.ROL_ROR_RCL_RCR_SAL_SAR_SHR_EB_DB,
        'ModRM_rm_111_00',
        'IMM_u8',
      ],
      operandSize: 8,
      cycles: 8,
    },
    {
      operation: [
        'next if (%{IMM} & 0x1f) == 0',
        'effective_address = ${MOD_RM_SEGMENT} + ${MOD_RM_OFFSET} + %{DISP}',
        'a = RAM:u8[effective_address]',
        'b = %{IMM} & 0x1f',
        '${ALU8_OP}',
        'RAM:u8[effective_address] = alu_result',
      ],
      undefined: ['AF', 'OF'],
      opcode: [
        Opcodes.ROL_ROR_RCL_RCR_SAL_SAR_SHR_EB_DB,
        'ModRM_rm_111_01',
        'DISP_i8',
        'IMM_u8',
      ],
      operandSize: 8,
      cycles: 8,
    },
    {
      operation: [
        'next if (%{IMM} & 0x1f) == 0',
        'effective_address = ${MOD_RM_SEGMENT} + ${MOD_RM_OFFSET} + %{DISP}',
        'a = RAM:u8[effective_address]',
        'b = %{IMM} & 0x1f',
        '${ALU8_OP}',
        'RAM:u8[effective_address] = alu_result',
      ],
      undefined: ['AF', 'OF'],
      opcode: [
        Opcodes.ROL_ROR_RCL_RCR_SAL_SAR_SHR_EB_DB,
        'ModRM_rm_111_10',
        'DISP_i16',
        'IMM_u8',
      ],
      operandSize: 8,
      cycles: 8,
    },
    {
      operation: [
        'next if (%{IMM} & 0x1f) == 0',
        'a = ${MOD_RM_RM8}',
        'b = %{IMM} & 0x1f',
        '${ALU8_OP}',
        '${MOD_RM_RM8} = alu_result',
      ],
      undefined: ['AF', 'OF'],
      opcode: [
        Opcodes.ROL_ROR_RCL_RCR_SAL_SAR_SHR_EB_DB,
        'ModRM_rm_111_11',
        'IMM_u8',
      ],
      operandSize: 8,
      cycles: 5,
    },
    // 0xD1 /7 - SAR ew, 1
    {
      operation: [
        'effective_address = ${MOD_RM_SEGMENT} + %{DISP}',
        'a = RAM:u16[effective_address]',
        'b = 1',
        '${ALU16_OP}',
        'RAM:u16[effective_address] = alu_result',
      ],
      modifies: ['OF', 'CF', 'ZF', 'PF', 'SF'],
      opcode: [
        Opcodes.ROL_ROR_RCL_RCR_SAL_SAR_SHR_EW,
        'ModRM_110_111_00',
        'DISP_i16',
      ],
      operandSize: 16,
      cycles: 7,
    },
    {
      operation: [
        'effective_address = ${MOD_RM_SEGMENT} + ${MOD_RM_OFFSET}',
        'a = RAM:u16[effective_address]',
        'b = 1',
        '${ALU16_OP}',
        'RAM:u16[effective_address] = alu_result',
      ],
      modifies: ['OF', 'CF', 'ZF', 'PF', 'SF'],
      opcode: [Opcodes.ROL_ROR_RCL_RCR_SAL_SAR_SHR_EW, 'ModRM_rm_111_00'],
      operandSize: 16,
      cycles: 7,
    },
    {
      operation: [
        'effective_address = ${MOD_RM_SEGMENT} + ${MOD_RM_OFFSET} + %{DISP}',
        'a = RAM:u16[effective_address]',
        'b = 1',
        '${ALU16_OP}',
        'RAM:u16[effective_address] = alu_result',
      ],
      modifies: ['OF', 'CF', 'ZF', 'PF', 'SF'],
      opcode: [
        Opcodes.ROL_ROR_RCL_RCR_SAL_SAR_SHR_EW,
        'ModRM_rm_111_01',
        'DISP_i8',
      ],
      operandSize: 16,
      cycles: 7,
    },
    {
      operation: [
        'effective_address = ${MOD_RM_SEGMENT} + ${MOD_RM_OFFSET} + %{DISP}',
        'a = RAM:u16[effective_address]',
        'b = 1',
        '${ALU16_OP}',
        'RAM:u16[effective_address] = alu_result',
      ],
      modifies: ['OF', 'CF', 'ZF', 'PF', 'SF'],
      opcode: [
        Opcodes.ROL_ROR_RCL_RCR_SAL_SAR_SHR_EW,
        'ModRM_rm_111_10',
        'DISP_i16',
      ],
      operandSize: 16,
      cycles: 7,
    },
    {
      operation: [
        'a = ${MOD_RM_RM16}',
        'b = 1',
        '${ALU16_OP}',
        '${MOD_RM_RM16} = alu_result',
      ],
      modifies: ['OF', 'CF', 'ZF', 'PF', 'SF'],
      opcode: [Opcodes.ROL_ROR_RCL_RCR_SAL_SAR_SHR_EW, 'ModRM_rm_111_11'],
      operandSize: 16,
      cycles: 2,
    },
    // 0xD3 /7 - SAR ew, CL
    {
      operation: [
        'next if (CL & 0x1f) == 0',
        'effective_address = ${MOD_RM_SEGMENT} + %{DISP}',
        'a = RAM:u16[effective_address]',
        'b = CL & 0x1f',
        '${ALU16_OP}',
        'RAM:u16[effective_address] = alu_result',
      ],
      undefined: ['AF', 'OF'],
      opcode: [
        Opcodes.ROL_ROR_RCL_RCR_SAL_SAR_SHR_EW_CL,
        'ModRM_110_111_00',
        'DISP_i16',
      ],
      operandSize: 16,
      cycles: 8,
    },
    {
      operation: [
        'next if (CL & 0x1f) == 0',
        'effective_address = ${MOD_RM_SEGMENT} + ${MOD_RM_OFFSET}',
        'a = RAM:u16[effective_address]',
        'b = CL & 0x1f',
        '${ALU16_OP}',
        'RAM:u16[effective_address] = alu_result',
      ],
      undefined: ['AF', 'OF'],
      opcode: [Opcodes.ROL_ROR_RCL_RCR_SAL_SAR_SHR_EW_CL, 'ModRM_rm_111_00'],
      operandSize: 16,
      cycles: 8,
    },
    {
      operation: [
        'next if (CL & 0x1f) == 0',
        'effective_address = ${MOD_RM_SEGMENT} + ${MOD_RM_OFFSET} + %{DISP}',
        'a = RAM:u16[effective_address]',
        'b = CL & 0x1f',
        '${ALU16_OP}',
        'RAM:u16[effective_address] = alu_result',
      ],
      undefined: ['AF', 'OF'],
      opcode: [
        Opcodes.ROL_ROR_RCL_RCR_SAL_SAR_SHR_EW_CL,
        'ModRM_rm_111_01',
        'DISP_i8',
      ],
      operandSize: 16,
      cycles: 8,
    },
    {
      operation: [
        'next if (CL & 0x1f) == 0',
        'effective_address = ${MOD_RM_SEGMENT} + ${MOD_RM_OFFSET} + %{DISP}',
        'a = RAM:u16[effective_address]',
        'b = CL & 0x1f',
        '${ALU16_OP}',
        'RAM:u16[effective_address] = alu_result',
      ],
      undefined: ['AF', 'OF'],
      opcode: [
        Opcodes.ROL_ROR_RCL_RCR_SAL_SAR_SHR_EW_CL,
        'ModRM_rm_111_10',
        'DISP_i16',
      ],
      operandSize: 16,
      cycles: 8,
    },
    {
      operation: [
        'next if (CL & 0x1f) == 0',
        'a = ${MOD_RM_RM16}',
        'b = CL & 0x1f',
        '${ALU16_OP}',
        '${MOD_RM_RM16} = alu_result',
      ],
      undefined: ['AF', 'OF'],
      opcode: [Opcodes.ROL_ROR_RCL_RCR_SAL_SAR_SHR_EW_CL, 'ModRM_rm_111_11'],
      operandSize: 16,
      cycles: 5,
    },
    // 0xC1 /7 - SAR ew, db
    {
      operation: [
        'next if (%{IMM} & 0x1f) == 0',
        'effective_address = ${MOD_RM_SEGMENT} + %{DISP}',
        'a = RAM:u16[effective_address]',
        'b = %{IMM} & 0x1f',
        '${ALU16_OP}',
        'RAM:u16[effective_address] = alu_result',
      ],
      undefined: ['AF', 'OF'],
      opcode: [
        Opcodes.ROL_ROR_RCL_RCR_SAL_SAR_SHR_EW_DB,
        'ModRM_110_111_00',
        'DISP_i16',
        'IMM_u8',
      ],
      operandSize: 16,
      cycles: 8,
    },
    {
      operation: [
        'next if (%{IMM} & 0x1f) == 0',
        'effective_address = ${MOD_RM_SEGMENT} + ${MOD_RM_OFFSET}',
        'a = RAM:u16[effective_address]',
        'b = %{IMM} & 0x1f',
        '${ALU16_OP}',
        'RAM:u16[effective_address] = alu_result',
      ],
      undefined: ['AF', 'OF'],
      opcode: [
        Opcodes.ROL_ROR_RCL_RCR_SAL_SAR_SHR_EW_DB,
        'ModRM_rm_111_00',
        'IMM_u8',
      ],
      operandSize: 16,
      cycles: 8,
    },
    {
      operation: [
        'next if (%{IMM} & 0x1f) == 0',
        'effective_address = ${MOD_RM_SEGMENT} + ${MOD_RM_OFFSET} + %{DISP}',
        'a = RAM:u16[effective_address]',
        'b = %{IMM} & 0x1f',
        '${ALU16_OP}',
        'RAM:u16[effective_address] = alu_result',
      ],
      undefined: ['AF', 'OF'],
      opcode: [
        Opcodes.ROL_ROR_RCL_RCR_SAL_SAR_SHR_EW_DB,
        'ModRM_rm_111_01',
        'DISP_i8',
        'IMM_u8',
      ],
      operandSize: 16,
      cycles: 8,
    },
    {
      operation: [
        'next if (%{IMM} & 0x1f) == 0',
        'effective_address = ${MOD_RM_SEGMENT} + ${MOD_RM_OFFSET} + %{DISP}',
        'a = RAM:u16[effective_address]',
        'b = %{IMM} & 0x1f',
        '${ALU16_OP}',
        'RAM:u16[effective_address] = alu_result',
      ],
      undefined: ['AF', 'OF'],
      opcode: [
        Opcodes.ROL_ROR_RCL_RCR_SAL_SAR_SHR_EW_DB,
        'ModRM_rm_111_10',
        'DISP_i16',
        'IMM_u8',
      ],
      operandSize: 16,
      cycles: 8,
    },
    {
      operation: [
        'next if (%{IMM} & 0x1f) == 0',
        'a = ${MOD_RM_RM16}',
        'b = %{IMM} & 0x1f',
        '${ALU16_OP}',
        '${MOD_RM_RM16} = alu_result',
      ],
      undefined: ['AF', 'OF'],
      opcode: [
        Opcodes.ROL_ROR_RCL_RCR_SAL_SAR_SHR_EW_DB,
        'ModRM_rm_111_11',
        'IMM_u8',
      ],
      operandSize: 16,
      cycles: 5,
    },
  ],
};
