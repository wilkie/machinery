...
class TestMachine extends Machine {
  ...
  private handleAssertions(vector: number): boolean {
    if (vector === 0x23) {
      if (this.AX !== this.BX) {
        console.log(
          `FAIL: AX=0x${this.AX.toString(16)} AH=0x${this.BX.toString(16)} IP=0x${(this.IP & 0xffff).toString(16)}`,
        );
        this.halted = -1;
      }
      return true;
      ...
      if (this.AL !== this.AH) {
        console.log(
          `FAIL: AL=0x${this.AL.toString(16)} AH=0x${this.AH.toString(16)} IP=0x${(this.IP & 0xffff).toString(16)}`,
        );
        this.halted = -1;
      }
      return true;
  ...
  interrupt_protected(vector: number) {
    if (!this.handleAssertions(vector)) {
      ...
      // Clear SOFTWARE_INT flag (mem8[272]) that was set by the INT instruction.
